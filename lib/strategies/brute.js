...
module.exports = function brute(transitions) {
  ...
  states.get(Immutable.Map()).forEach(function(state, name) {
    stack.push(Immutable.Map({
      state: state,
      path: Immutable.List([name])
    }));
  });
  ...
  while (stack.length > 0) {
    ...
    var nonVisited = outbound.filter(function(state, name) {
      return !edgesFollowed.has(Immutable.Map({
          source: state,
          name: name,
          destination: node.get('state')
        }));
    });
    ...
    if (nonVisited.isEmpty()) {
      ...
      nonVisited.forEach(function(state, name) {
        edgesFollowed = edgesFollowed.add(Immutable.Map({
          source: state,
          name: name,
          destination: node.get('state')
        }));
